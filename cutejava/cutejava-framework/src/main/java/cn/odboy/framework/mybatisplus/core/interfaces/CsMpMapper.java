 ...
 * @date 2022-04-03
 */
public interface CsMpMapper<E> extends BaseMapper<E> {
    /**
     * 批量插入
     *
     ...
     * @return IPage<T>
     */
    default <T> CsPageResult<T> queryFeatureClazzPage(Pageable pageable, LambdaQueryWrapper<E> wrapper, Class<T> clazz) {
        ...
        if (CollUtil.isEmpty(pageInfo.getRecords())) {
            return new CsPageResult<>(new ArrayList<>(), pageInfo.getTotal());
        }
        return new CsPageResult<>(BeanUtil.copyToList(pageInfo.getRecords(), clazz), pageInfo.getTotal());
    }
